############
# Step28BYJ48.py  a Micropython module for the 28BYJ48 stepper motor
#
...
class Step28BYJ48:
	...
	def __init__(self, pin1, pin2, pin3, pin4, timer=None):
		...
		"""
		self._drive = [
			pyb.Pin(pin1, pyb.Pin.OUT_PP),
			pyb.Pin(pin2, pyb.Pin.OUT_PP),
			pyb.Pin(pin3, pyb.Pin.OUT_PP),
			pyb.Pin(pin4, pyb.Pin.OUT_PP)
		]
		if timer == None:
			timer = 5
	...
	def asyncSteps(self, n, callback = None):
		...
		self._timer.init(freq=self._phaseFreq(), callback=lambda t: self._onePhase())
