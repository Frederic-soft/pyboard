############
# BME280.py a Micropython pyboard driver for Bosch BME280 combined humidity, 
...
class BME280 :
  ...
  """
  Initialize a BME280 sensor on I2C bus i2c_num (I2C 1 has SCL on X9 and SDA on X10,
  I2C 2 has SCL on Y9 and SDA on Y10), at the given address on the I2C bus.
  """
  def __init__(self, i2c_num, address = 0x76) :
    ...
    self.id = BME280.int_from_bytes(self.i2c.mem_read(1, self.addr, self.ID_REG))
    ...
    self.dig_H1 = BME280.int_from_bytes(self.i2c.mem_read(1, self.addr, BME280.DIG_H1), "little")
    self.i2c.mem_read(buf, self.addr, BME280.DIG_H2)
    self.dig_H2 = BME280.int_from_bytes(buf, "little", signed = True)
    self.dig_H3 = BME280.int_from_bytes(self.i2c.mem_read(1, self.addr, BME280.DIG_H3), "little")
    ...
    self.dig_H6 = BME280.int_from_bytes(self.i2c.mem_read(1, self.addr, BME280.DIG_H6), "little", signed = True)
  ...
  def oneshot(self) :
    ...
    while (self.i2c.mem_read(1, self.addr, BME280.STAT_REG)[0]
  ...
  def normalmode(self, standby = 0) :
    ...
    self.i2c.mem_read(buf, self.addr, BME280.CONF_REG)
  ...
  def filtering(self, coef) :
    ...
    self.i2c.mem_write(buf, self.addr, BME280.CONF_REG)
  ...
  def humidity_mode(self, mode) :
    ...
    buf = bytearray(1)
    self.i2c.mem_read(buf, self.addr, BME280.HUM_CTRL_REG)
    buf[0] &= ~BME280.HUM_MASK
    buf[0] |= mode
    self.i2c.mem_write(buf, self.addr, BME280.HUM_CTRL_REG)
  ...
  def raw_measure(self, buf) :
    self.i2c.mem_read(buf, self.addr, self.PRESS_REG) # Read pressure, temperature and humidity all at once
    return buf
