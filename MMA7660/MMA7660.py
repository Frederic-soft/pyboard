############
# MMA7660.py  a Micropython module for the MMA7660FC accelerometer of the pyboard
#
...
class MMA7660:
	...
	"""
	def __init__(self):
		self.i2c = pyb.I2C(1, pyb.I2C.MASTER) # MMA is on I2C bus 1
		self.power_pin = pyb.Pin('MMA_AVDD')
		self.power_pin.low()                  # switch off MMA power
		prev_devices = self.i2c.scan()        # get devices on I2C bus
		self.power_pin.high()                 # switch on MMA power
		devices = self.i2c.scan()             # get devices on I2C bus
		self.address = None
		for dev in devices:
			if not dev in prev_devices:       # the new device is the MMA7660
				self.address = dev
				break
	...
	* bit is the mask of the bit to set
	...
	def setBit(self, reg, bit):
		self.i2c.mem_read(self.buf, self.address, reg)  # get previous register value
		self.buf[0] |= bit                              # set bit to 1
		self.i2c.mem_write(self.buf, self.address, reg) # write new value
	...
	* on: True for active mode, False for standby mode
	...
	"""
	def isOn(self):
		mode = self.i2c.mem_read(1, self.address, MMA7660.MODE_REG)
	...
	* data: a bytearray of length 3
	...
	def getSample(self, data):
		self.i2c.mem_read(data, self.address, MMA7660.X_REG)  # read sample
		for i in range(3):
	...
	* rate: one of AMPD, AM64, ..., AM2, AM1
	...
	def setActiveSamplingRate(self, rate):
		...
		self.i2c.mem_write(self.buf, self.address, MMA7660.SAMPLING_REG)
	...
	* mask: any combination of SHINTX, SHINTY, ..., PLINT, FBINT
	...
	def enableInterrupt(self, mask):
		...
		self.i2c.mem_write(mask, self.address, MMA7660.INT_REG)
		self.on(was_on)       # restore previous mode
